...
abstract class PolyOp0 extends PolyFunc0 {
  ...
  override def apply[F[_]: Algebra, Y]()(implicit f: P[Y]) = f()

  override def ground[Y](implicit f: P[Y]) = f
}
...
abstract class PolyOp1 extends PolyFunc1 {
  ...
  override def apply[F[_]: Algebra, X, Y](x: F[X])(implicit f: P[X, Y]) = f(x)

  def ground[X, Y](implicit f: P[X, Y]) = f
...
abstract class PolyOp2 extends PolyFunc2 { self =>
  ...
  override def apply[F[_]: Algebra, X1, X2, Y](x1: F[X1], x2: F[X2])(implicit f: P[X1, X2, Y]) = f(x1, x2)

  def ground[X1, X2, Y](implicit f: P[X1, X2, Y]) = f

  object Curried1 extends ParameterizedPolyOp1 {

    implicit def curriedLiteral1[X1, X2, Y](implicit f: self.P[X1, X2, Y]): X1 => P[X2, Y] = (x1: X1) =>
      new P[X2, Y] {
  ...
  object Curried2 extends ParameterizedPolyOp1 {
    implicit def curriedLiteral2[X1, X2, Y](implicit f: self.P[X1, X2, Y]): X2 => P[X1, Y] = (x2: X2) =>
      new P[X1, Y] {
...
abstract class PolyOp3 extends PolyFunc3 {
  ...
  override def apply[F[_]: Algebra, X1, X2, X3, Y](x1: F[X1], x2: F[X2], x3: F[X3])(implicit f: P[X1, X2, X3, Y]) = f(x1, x2, x3)

  def ground[X1, X2, X3, Y](implicit f: P[X1, X2, X3, Y]) = f
}
