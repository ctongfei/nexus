...
object Merge {
  ...
  implicit def case0[T <: HList, U, V, W]: Aux[U :: V :: T, U, V, W, W :: T] =
    new Merge[U :: V :: T, U, V, W] { m =>
      type Out = W :: T
      type Index = _0
      def index = 0
      def inverse = Split.case0
    }
  ...
  (implicit m: Merge.Aux[T, U, V, W, R]): Merge.Aux[H :: T, U, V, W, H :: R] =
    new Merge[H :: T, U, V, W] {
      ...
      def index = m.index + 1
      def inverse = Split.caseN(m.inverse)
    }
  ...
  (implicit al: ToHList.Aux[A, Al], m: Merge.Aux[Al, U, V, W, Bl], bl: FromHList.Aux[Bl, B]): Merge.Aux[A, U, V, W, B] =
    new Merge[A, U, V, W] {
      ...
      type Index = m.Index
      def index = m.index
      def inverse = Split.tuple(bl.inverse, m.inverse, al.inverse)
    }
