...
object Split {
  ...
  implicit def splitHListCase0[T <: HList, U, V, W]: Aux[U :: T, U, V, W, V :: W :: T] =
    new Split[U :: T, U, V, W] { s =>
      type Out = V :: W :: T
      type Index = _0
      def index = 0
      def inverse = Merge.mergeHListCase0
    }

  implicit def splitHListCaseN[T <: HList, H, U, V, W, R <: HList]
  (implicit s: Split.Aux[T, U, V, W, R]): Aux[H :: T, U, V, W, H :: R] =
    new Split[H :: T, U, V, W] {
      type Out = H :: R
      type Index = Succ[s.Index]
      def index = s.index + 1
      def inverse = Merge.mergeHListCaseN(s.inverse)
    }

  implicit def splitTuple[A, Al <: HList, U, V, W, Bl <: HList, B]
  (implicit al: ToHList.Aux[A, Al], s: Split.Aux[Al, U, V, W, Bl], bl: FromHList.Aux[Bl, B]): Aux[A, U, V, W, B] =
    new Split[A, U, V, W] {
      type Out = B
      type Index = s.Index
      def index = s.index
      def inverse = Merge.mergeTuple(bl.inverse, s.inverse, al.inverse)
    }
