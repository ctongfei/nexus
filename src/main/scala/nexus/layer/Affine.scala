...
object Affine {
  ...
  def apply[T[_, _ <: $$], D, A, B](in: (A, Int), out: (B, Int))(implicit env: Env[T, D]) = {
    ...
    val (aA, nA) = in
    val (aB, nB) = out
    val W = Param(newGaussianTensor(0f, 1f, aB::aA::$, Array(nB, nA)), name = s"Affine$i.W")
    val b = Param(newGaussianTensor(0f, 1f, aB::$, Array(nB)), name = s"Affine$i.b")
    i += 1
