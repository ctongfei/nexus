...
)(implicit val ops: IsTypedRealTensor[T, D])
...
object Linear {
  ...
  def from[T[_ <: $$], D, A, B]
  (W: Param[T[B::A::$]])(implicit ops: IsTypedRealTensor[T, D]) = new Linear[T, D, A ,B](W)

  /**
   * Constructs a linear layer with default parameters.
   * @example  Linear(In -> 784, Out -> 200)
   */
  def apply[T[_ <: $$], D, A, B](in: (A, Int), out: (B, Int))(implicit ops: IsTypedRealTensor[T, D]): Unit = {
    import ops._
    val (aA, nA) = in
    val (aB, nB) = out
