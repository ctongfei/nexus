 ...
 * @author Tongfei Chen
 */
trait TypedRealTensorOps[T[_ <: $$], R] extends TypedTensorOps[T, R] with AxisTyping[T] { self =>
  ...
  def ground[A <: $$]: GradOps[T[A]] = new GradOps[T[A]] {
    ...
    def addS(x1: T[A], x2: Double) = self.addS(x1, D.fromDouble(x2))
    ...
    def scale(x: T[A], k: Double) = self.scale(x, D.fromDouble(k))
