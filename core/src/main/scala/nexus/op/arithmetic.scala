...
object Mul extends PolyOp2 {
  ...
  implicit def mulF[R: IsReal]: F[R, R, R] = new F[R, R, R] {
    def name = "Mul"
  ...
  object Elementwise extends PolyOp2 {
    implicit def mulElementwiseF[T[_], R, A](implicit T: IsRealTensorK[T, R]): F[T[A], T[A], T[A]] = new F[T[A], T[A], T[A]] {
...
object Div extends PolyOp2 {
  ...
  implicit def divF[R: IsReal]: F[R, R, R] = new F[R, R, R] {
    def name = "Div"
    def tag(tx1: Type[R], tx2: Type[R]) = tx1
    def forward(x1: R, x2: R) = x1 / x2
    def backward1(dy: R, y: R, x1: R, x2: R) = dy / x2
...
object Neg extends PolyOp1 {
  ...
  implicit def negF[X: Grad]: F[X, X] = new F[X, X] {
    def name = "Neg"
    def tag(tx: Type[X]) = tx
    def forward(x: X) = -x
    def backward(dy: X, y: X, x: X) = -dy
...
object Inv extends PolyOp1 {
  ...
  implicit def invF[R](implicit R: IsReal[R]): F[R, R] = new F[R, R] {
    def name = "Inv"
